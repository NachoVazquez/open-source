...
type AddressCmd = AddressKeyGen | AddressKeyHash | AddressBuild | AddressBuildMultiSig | AddressInfo;

const renderAddressCmd = (addressCmd: AddressCmd) => {
    if (addressCmd as AddressKeyGen)
        return "address key-gen"
    if (addressCmd as AddressKeyHash)
        return "address address key-hash"
    if (addressCmd as AddressBuild)
        return "address build"
    if (addressCmd as AddressBuildMultiSig)
        return "address build-script"
    if (addressCmd as AddressInfo)
        return "address info"
}
